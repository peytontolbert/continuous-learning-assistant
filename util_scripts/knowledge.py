...
def add_retrieval_knowledge(text, embedding_model, index, retrieval_knowledge_store, retrieval_knowledge_set):
    ...
    try:
        with knowledge_lock:  # {{ Ensure thread safety }}
            ...
            if text in retrieval_knowledge_set:
                ...
                return
            embedding = embedding_model.encode([text]).astype('float32')  # Ensure float32 for FAISS
            index.add(embedding)
...
def save_training_knowledge_store(training_knowledge_store, training_knowledge_store_file):
...
def save_all(index, retrieval_faiss_index_file, training_knowledge_store, training_knowledge_store_file, retrieval_knowledge_store, retrieval_knowledge_set):
    logging.debug("Saving all knowledge stores.")
    save_retrieval_faiss_index(index, retrieval_faiss_index_file)
    save_training_knowledge_store(training_knowledge_store, training_knowledge_store_file)
    # Save retrieval knowledge store
    logging.debug(f"Saving retrieval knowledge store to {RETRIEVAL_KNOWLEDGE_STORE_FILE}")
    try:
...
def get_relevant_knowledge(query, embedding_model, index, retrieval_knowledge_store, top_k=5):
    ...
    try:
        ...
        relevant_knowledge = [retrieval_knowledge_store[idx] for idx in indices[0] if idx < len(retrieval_knowledge_store)]
...
def extract_knowledge(conversation_history, preferred_response):
    ...
    try:
        ...
        if preferred_response:
            conversation_history.append({"role": "user", "content": "[PREFERRED RESPONSE]\n" + preferred_response})
        ...
        try:
            ...
            semantic_response = outputs[0]["generated_text"].strip()
            logging.debug(f"Generated semantic_response: {semantic_response}")
        except Exception as e:
            logging.error(f"Error generating semantic response: {e}")
            semantic_response = "I'm sorry, I encountered an error while generating semantic knowledge."
        ...
        try:
            ...
            episodic_response = outputs[0]["generated_text"].strip()
            logging.debug(f"Generated episodic_response: {episodic_response}")
        except Exception as e:
            logging.error(f"Error generating episodic response: {e}")
            episodic_response = "I'm sorry, I encountered an error while generating episodic knowledge."
...
def load_retrieval_knowledge_store(file_path):
    ...
    try:
        ...
        logging.info("Retrieval knowledge store loaded with set for duplication checks.")
